...
export function BonusCashoutPanel() {
  ...
  const [availableBonus, setAvailableBonus] = useState<number>(0);
  ...
  const loadWallet = (
    ...
    const cached = getCachedApi<BackendWallet>(`/wallet/${encodeURIComponent(nextUserId)}`);
    if (cached) {
      const balance = calculateBonusBalance(cached);
  ...
  useEffect(() => {
    ...
    loadCurrentUserWallet({ showError: true });

    const interval = window.setInterval(() => {
      if (currentUserId) {
        loadWallet(currentUserId, { showError: false });
      }
    }, 15000);
    ...
    const handleBonusUpdate = (event: Event) => {
      ...
      const updatedUserId = customEvent.detail?.userId ?? currentUserId;
      if (updatedUserId) {
        loadWallet(updatedUserId, { showError: false });
      }
    };
  ...
  const submitCashout = async () => {
    ...
    try {
      ...
      setSubmittedPopup({
        title: "Bonus Cashout Submitted",
        message: `Your bonus cashout request of ${formatCurrency(parsedAmount, "NGN")} has been received.`,
        detail: "Our admin team will review and process your bonus payout to your provided bank account shortly.",
      });
    } catch (error) {
      setFeedback(
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">
              Request a payout from your available referral and threshold bonus balance anytime (no minimum amount restriction).
            </p>
          </div>
